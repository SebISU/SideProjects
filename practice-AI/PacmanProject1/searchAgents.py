...
def cornersHeuristic(state, problem):
    ...
    remCorners = [corner for corner in corners if corner not in state[1]]
    heurVal = 0
    currPosition = state[0]

    while len(remCorners):
        distCorners = [(util.manhattanDistance(currPosition, corner), corner) for corner in remCorners]

        currDistance, currCorner = min(distCorners)
        heurVal += currDistance
        currPosition = currCorner
        remCorners.remove(currCorner)
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        ...
        distFood.append(problem.heuristicInfo[(position, food)])
